...
from agno.tools.eleven_labs import OpenAIChat
...
if generate_button:
    if url.strip() == "":
        ...
        # Set API keys as enviroment varibles for Agno and Tools
    ...
    with st.spinner("Processing... Scraping blog, summarizing and generating podcast 🎶"):
        try:
            blog_to_podcast_agent = Agent(
                ...
                desciption="you are an AI agent that can generate audio using the ElevenLabs API.",
                instructions=[
                    "When the user provides a blog URL:",
                    "1. Use FirecrawTools to scrape the blog content",
            ...
            if podcast.audio and len(podcast.audio) > 0:
                ...
                st.success("Poadcast generated successfully! 🎧")
                audio_byte = open(filename, "rb").read()
